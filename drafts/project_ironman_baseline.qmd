...
The thing is, I've caught the bug again, and so I find myself wishing complete an Ironman sometime before I die. I've got plenty of life left to live, but I'd prefer to complete one in a respectable way [^3], within my physical prime. But what does this even mean?
...
Alright, we now know what seems to be a reasonable enough time to finish an Ironman 70.3 across *all age groups*. But does age matter at all? Do i have all the time in the world to train for this if my goal is to just be above average? 

```{python}

#for each 
contingency_table = pd.crosstab(df_amateur_workingage['Performance_category'], df_amateur_workingage['AgeGroup'])
contingency_norm = round(contingency_table.div(contingency_table.sum(axis =1), axis = 0)*100,2)
contingency_table = contingency_table.reset_index()
norm_contingency_tbl = tabulate(contingency_norm, headers='keys', tablefmt='pipe', showindex = False)
Markdown(norm_contingency_tbl)
...
# Create dummy variables for AgeGroup with drop_first=True
age_group_dummies = pd.get_dummies(df_amateur_workingage['AgeGroup'], drop_first=True, prefix='AgeGroup')

# Fit multinomial logistic regression model
multinomial = sm.MNLogit(df_amateur_workingage['Performance_category'], age_group_dummies)
result = multinomial.fit()
# Get unique age groups
all_agegrp = df_amateur_workingage['AgeGroup'].unique()
df_agegrptest = pd.get_dummies(pd.Series(all_agegrp), drop_first =True)
# Predict probabilities for each age group
predicted_probs = result.predict(df_agegrptest)

# Add age group labels to predicted probabilities dataframe
predicted_probs['AgeGroup'] = all_agegrp
# Define the desired order of age groups
desired_order = ['18-24', '25-29', '30-34', '35-39', '40-44', '45-49', '50-54', '55-59', '60-64']

# Reorder predicted_probs dataframe based on desired order of age groups
predicted_probs['AgeGroup'] = pd.Categorical(predicted_probs['AgeGroup'], categories=desired_order, ordered=True)
predicted_probs = predicted_probs.sort_values(by='AgeGroup')
